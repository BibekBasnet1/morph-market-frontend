...
async function fetchTermsHtml(): Promise<string> {
  const base = import.meta.env.BASE_URL || "/";
  const path = `${base.endsWith("/") ? base : `${base}/`}terms-of-service.html`;
  const res = await fetch(path);
  if (!res.ok) {
    throw new Error("Could not load terms of service file.");
  }
  return res.text();
}
...
export default function TermsOfServicePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["terms-of-service-static"],
    queryFn: fetchTermsHtml,
  ...
  if (isLoading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-3 text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-black">
        ...
        <p className="text-sm">Loading terms of service...</p>
      </div>
    );
  }
      ...
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 px-4 text-center bg-gray-50 dark:bg-black">
        ...
        <div className="space-y-2 max-w-md">
          <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            Could not load terms of service
          </h1>
