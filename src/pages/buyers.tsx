...
const columns: ColumnDef<Buyer>[] = [
  ...
  },
  {
    ...
    header: 'Status',
    render: (buyer) => (
      <Badge variant={buyer.status === 'active' ? 'default' : buyer.status === 'inactive' ? 'secondary' : 'destructive'}>
        {buyer.status}
      </Badge>
    ),
  },
  ...
  useEffect(() => {
    ...
    if (filters.search) {
      ...
      result = result.filter(
        buyer => buyer.name.toLowerCase().includes(search) || buyer.email.toLowerCase().includes(search)
      );
    }
