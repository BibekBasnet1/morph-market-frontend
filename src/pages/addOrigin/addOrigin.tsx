  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        ...
        const created = await OriginService.create(formData);
        return created;
      }
    },
    onSuccess: (origin: Origin) => {
      queryClient.setQueryData<Origin[]>(["origins"], old =>
        editingId
          ? old!.map(o => (o.id === editingId ? origin : o))
          : [...(old || []), origin]
      );

      toast.success(editingId ? "Origin updated" : "Origin created");
      resetForm();
      setIsOpen(false);
    },
 onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: OriginService.remove,
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openAddModal}>+ Add Origin</Button>
      </div>
            ...
            <CardContent className="p-4 flex items-start justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="destructive"
                  className="flex justify-center items-center"
                  onClick={() => openDeleteModal(origin.id)}
                >
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-lg p-6">
        <h3 className="mb-4 text-lg font-semibold">
          {editingId ? "Edit Origin" : "Add Origin"}
        </h3>
